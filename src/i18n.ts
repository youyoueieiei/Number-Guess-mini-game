const translations = {
  en: {
    eyebrow: (n: number) => `${n}-digit code breaker`,
    title: 'Number Guessing Game',
    description: (n: number) => `Guess a ${n} digit secret number. Repeated digits are allowed, and every guess shows how many digits are present plus how many are in the right place.`,
    players: 'Players',
    digits: 'Digits',
    difficulty: 'Difficulty',
    language: 'Language',
    gameOptions: 'Game options',
    currentLength: (n: number) => `Current game: ${n} digits`,
    nextLength: (n: number) => `Next game: ${n} digits`,
    lengthPending: 'Changes apply when you start a new game.',
    hardHint: 'Hard = position count only (number of digits in the correct position).',
    feedbackLabelEasy: 'present / position',
    solo: 'Solo',
    twoPlayers: '2 players',
    soloTitle: 'Solo mode',
    twoPlayerTitle: '2 player mode',
    twoPlayerSubtitle: 'Players A and B can each set a private secret and type guesses. Top = Player A, Bottom = Player B.',
    easy: 'Easy',
    hard: 'Hard',
    newGame: 'New game',
    guess: 'Guess',
    yourGuess: 'Your guess',
    playerLabel: (p: string) => `Player ${p}`,
    playerEnterGuess: (p: string) => `Player ${p}, enter your guess`,
    guessHistory: 'Guess history',
    guesses: (n: number) => `${n} guess${n === 1 ? '' : 'es'}`,
    playerGuesses: (player: string, n: number) => `Player ${player}: ${n} guess${n === 1 ? '' : 'es'}`,
    round: (n: number) => `Round ${n}`,
    noGuesses: (n: number) => `No guesses yet. Try a ${n} digit number to start.`,
    noGuessesShort: 'No guesses yet.',
    secretMaskedLabel: 'Secret number (masked)',
    guessLabel: (target: string) => `Guess ${target}'s number`,
    targetSecretNotSet: 'Target secret not set.',
    setSecretsFirst: 'Both players must set their secret numbers before guessing.',
    setSecretsButton: 'Set secrets',
    secretsLocked: 'Secrets locked — start guessing.',
    lockMySecret: 'Lock my secret',
    mySecretLocked: 'Game number locked',
    foundSolo: (secret: string, rounds: number, lang?: string) =>
      `You found ${secret} in ${rounds} round${rounds === 1 ? '' : 's'}!`,
    foundTwoPlayer: (player: string, target: string, rounds: number) =>
      `Player ${player} guessed Player ${target}'s number in ${rounds} round${rounds === 1 ? '' : 's'}!`,
  },
  zh: {
    eyebrow: (n: number) => `${n} 位數碼破譯`,
    title: '數字猜謎遊戲',
    description: (n: number) => `猜一個 ${n} 位數的秘密數字。可重複數字，每次猜測會顯示有多少數字存在，以及多少位置正確。`,
    players: '人數',
    digits: '位數',
    difficulty: '難度',
    language: '語言',
    gameOptions: '遊戲選項',
    currentLength: (n: number) => `目前局數：${n} 位`,
    nextLength: (n: number) => `下一局：${n} 位`,
    lengthPending: '位數變更會在新遊戲開始時套用。',
    hardHint: 'Hard：只顯示「正確位置」的個數（在正確位置上的數字個數）。',
    feedbackLabelEasy: '存在 / 位置',
    solo: '單人',
    twoPlayers: '雙人',
    soloTitle: '單人模式',
    twoPlayerTitle: '雙人模式',
    twoPlayerSubtitle: '玩家 A 與玩家 B 各自設定秘密數字，然後猜對方的數字。上方為玩家 A，下方為玩家 B。',
    easy: '簡單',
    hard: '困難',
    newGame: '新遊戲',
    guess: '猜測',
    yourGuess: '你的猜測',
    playerLabel: (p: string) => `玩家 ${p}`,
    playerEnterGuess: (p: string) => `玩家 ${p}，輸入你的猜測`,
    guessHistory: '猜測紀錄',
    guesses: (n: number) => `${n} 次猜測`,
    playerGuesses: (player: string, n: number) => `玩家 ${player}：${n} 次`,
    round: (n: number) => `第 ${n} 回合`,
    noGuesses: (n: number) => `尚未有猜測。請輸入 ${n} 位數字開始。`,
    noGuessesShort: '尚未有猜測。',
    secretMaskedLabel: '祕密數字（隱藏）',
    guessLabel: (target: string) => `猜測 ${target} 的數字`,
    targetSecretNotSet: '對方尚未設定秘密數字。',
    setSecretsFirst: '請雙方先設定秘密數字，然後再開始猜測。',
    setSecretsButton: '設定秘密數字',
    secretsLocked: '秘密已鎖定 — 開始猜測。',
    lockMySecret: '鎖定我的秘密',
    mySecretLocked: '遊戲數字已鎖定',
    foundSolo: (secret: string, rounds: number) => `你在 ${rounds} 回合內找到了 ${secret}！`,
    foundTwoPlayer: (player: string, target: string, rounds: number) =>
      `玩家 ${player} 在 ${rounds} 回合內猜中玩家 ${target} 的數字！`,
  },
};

export default translations;

